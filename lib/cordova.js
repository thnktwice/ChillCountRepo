...
var bleManager = function() {
  ...
  var reconnectSuccess = function (successReturn) {
    if (successReturn.status === "connected") {
      ...
      if (device.platform === iOSPlatform) {
        console.log("Reconnected on IOS ! good to go for battery");
  ...
  var suscribtionToBatterySuccess = function (successReturn) {
    if (successReturn.status === "suscribed") {
      console.log("suscribed to battery pop pop");
    }
    else if (successReturn.status === "suscribedResult") {
      console.log("Battery suscribtion..." + successReturn.value);
  ...
  var characteristicsBatterySuccess = function (successReturn) {
    if (successReturn.status == "discoveredCharacteristics"){
      ...
      var characteristics = successReturn.characteristics;
      console.log(JSON.stringify(successReturn));
      for (var i = 0; i < characteristics.length; i++){
        console.log("Battery Characteristics found");
        var characteristicUuid = characteristics[i].characteristicUuid;
        if (characteristicUuid === batteryLevelCharacteristicUuid) {
          console.log("Battery level characteristic found, suscribing...");
          var params = {"serviceUuid":batteryServiceUuid, "characteristicUuids":batteryLevelCharacteristicUuid, "isNotification": true};
          bluetoothle.suscribe(suscribtionToBatterySuccess,reconnectError,params);
        }
      }
    }
  };
  ...
  var beanBatteryService = function (successReturn) {
    if (successReturn.status === 'discoveredServices') {
      ...
      console.log("Unexpected services battery: " + successReturn.status);
      disconnectDevice();      
    }
  };
